...
bool pwma_config_enable(volatile avr32_pwma_t *pwma, uint32_t pwma_freq_hz, uint32_t gclk_freq_hz,
                        uint16_t spread)
{
    CHECK(pwma != NULL);
    return mock().actualCall("pwma_config_enable").returnBoolValue();
}
                              ...
                              uint16_t *channel_duty_cycles)
{
    ...
    return mock().actualCall("pwma_set_multiple_values").returnBoolValue();
}

bool pwma_write_top_value(volatile avr32_pwma_t *pwma, uint16_t top_value)
{
    CHECK(pwma != NULL);
    return mock().actualCall("pwma_write_top_value").returnBoolValue();
}
...
TEST(HalPwmTests, InitPwmCallsFunctions)
{
    ...
    mock().expectOneCall("pwma_config_enable").andReturnValue(static_cast<bool>(PASS));
    mock().expectOneCall("pwma_set_multiple_values").andReturnValue(static_cast<bool>(PASS));
    mock().expectOneCall("pwma_write_top_value").andReturnValue(static_cast<bool>(PASS));
    init_pwm_at32uc3l0256();
}
...
{
    mock().expectOneCall("gpio_enable_module").andReturnValue(GPIO_SUCCESS);
    mock().expectOneCall("pwma_config_enable").andReturnValue(static_cast<bool>(FAIL));
    mock().expectOneCall("RUNTIME_ERROR")
            .withUnsignedIntParameter("timestamp", 0)
            .withStringParameter("fail_message",
                                 "pwm init: configure_frequency_and_spread() failed")
...
TEST(HalPwmTests, InitPwmSetDutyCyclesFailureCallsRuntimeError)
{
    mock().expectOneCall("gpio_enable_module").andReturnValue(GPIO_SUCCESS);
    mock().expectOneCall("pwma_config_enable").andReturnValue(static_cast<bool>(PASS));
    mock().expectOneCall("pwma_set_multiple_values").andReturnValue(static_cast<bool>(FAIL));
    mock().expectOneCall("RUNTIME_ERROR")
            .withUnsignedIntParameter("timestamp", 0)
...
TEST(HalPwmTests, InitPwmSetTopFailureCallsRuntimeError)
{
    ...
    mock().expectOneCall("pwma_config_enable").andReturnValue(static_cast<bool>(PASS));
    mock().expectOneCall("pwma_set_multiple_values").andReturnValue(static_cast<bool>(PASS));
    mock().expectOneCall("pwma_write_top_value").andReturnValue(static_cast<bool>(FAIL));
    mock().expectOneCall("RUNTIME_ERROR")
            .withUnsignedIntParameter("timestamp", 0)
...
TEST(HalPwmTests, SetPwmDutyCycleCallsFunctions)
{
    ...
    set_pwm_duty_cycle_byte_at32uc3l0256(&wheel_motor_1, 10);
}
...
TEST(HalPwmTests, SetPwmDutyCycleSetDutyCyclesFailureCallsRuntimeError)
{
    ...
    set_pwm_duty_cycle_byte_at32uc3l0256(&wheel_motor_1, 0);
}
